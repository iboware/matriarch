...
//NewStatefulSet creates a statefulSet of PostgreSQL
func NewStatefulSet(cr *databasev1alpha1.PostgreSQL) *appsv1.StatefulSet {
	var log = logf.Log.WithName("controller_postgresql")

	labels := utils.NewLabels("postgres", cr.ObjectMeta.Name, "postgres")
	envVars := EnvVarsForStatefulSet(cr.ObjectMeta.Name, int(cr.Spec.Replicas), cr.Spec.Namespace, cr.Spec.EnablePgPool)
...
//EnvVarsForStatefulSet Creates Environmental Variables for PostgreSQL StatefulSet
func EnvVarsForStatefulSet(releaseName string, replicaCount int, namespace string, pgPool bool) []v1.EnvVar {

	var partnerNodes string
	var trustAll string = "no"
